...
class FcFlow(tf.keras.Model):
            ...
            for b in range(size_batch):
                ...
                for h in range(horizon):
                    ...
                    for ts in range(num_time_series):
                        ...
                        dim_result.append(bs.bootstrap(samples, stat_func=bs_stats.mean))
                    horizon_result.append(dim_result)
                batch_result.append(horizon_result)
            pred_result = pred_result+batch_result
        return pred_result
        ...
        for input_batch_dict, output_batch_dict in data_iter:
            ...
            for b in range(size_batch):
                for h in range(horizon):
                    ...
                    crpsum.append(self.crpsum(
                        multivariate_sample, multivariate_true_val))
           

        return np.mean(crpsum)

    def univariate_crp(self, multivariate_sample, multivariate_true_val):
        univar_crp = []
    ...
    def crpsum(self, multivariate_sample, multivariate_true_val):
        sample_sum = np.sum(multivariate_sample, axis=1)
        true_val_sum = np.sum(multivariate_true_val)
        return ps.crps_ensemble(true_val_sum, sample_sum)
